...
def delete_book(book_id):
    try:
        ...
        current_app.logger.debug(f"Delete operation result: {result.deleted_count} for book_id: {book_id}")
        if result.deleted_count == 0:
            ...
            response = jsonify({"ID": book_id})
            #response = jsonify({"status": "Book deleted", "id": book_id})
            current_app.logger.debug(f"Returning response: {response.data}")
            return response, 200
    except Exception as e:
